	...
	<view class="template-address">
		...
		<view class="">
			...
			<view v-for="(itme,index) in list" :key="index"
				...
				<view class="justify-content-item tn-text-xl tn-color-gray" @click="tn('./edit?id='+itme.id)">
			...
			<view class="tn-flex tn-footerfixed">
				<view class="tn-flex-1 justify-content-item tn-margin-right tn-margin-left tn-text-center">
					<tn-button backgroundColor="#07C160 " padding="40rpx 0" width="100%" :fontSize="28"
						fontColor="#FFFFFF" shape="round" @click="tn('./edit')">
			...
			del(id) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							console.log('确认')
							than.$http.postRequest('Address/del', {
									id: id
								})
								.then(res => {
									than.getlist()
